...
# --- Configuration ----
# ======================
DATASET_REPO_ID = "fiqa"   # 출력용 명칭
dataset = "fiqa"          # BEIR 내부 이름
...
def _rerank_task_with_grouped_gemm(retriever: ColbertFdeRetrieverNaive, task: "RerankTask", top_k: int):
    ...
    t_span_time = time.perf_counter() - t_span_0
    ...
    if tgt_cols <= 0:
        ...
        make_panel_start_time = time.perf_counter()
        panels = _make_panels(compute_ids, doc_blocks, tgt_cols)
        # print(f"[TPack Panel] formed {len(panels)} panels (target_cols={tgt_cols})")
        panel_cols = [sum(int(doc_blocks[did].shape[1]) for did in p) for p in panels]
        make_panel_time = time.perf_counter() - make_panel_start_time
        ...
        def _score_one_panel(panel_ids: List[str]) -> Tuple[List[Tuple[str, float]], int, float]:
            ...
            # print( f"[TPack Panel] panel_length={len(panel_ids)} copy_dt={copy_dt:.3f}s compute_dt={compute_dt:.3f}s" )
            return scores, bytes_need, copy_dt
        ...
        with ThreadPoolExecutor(max_workers=parallelism, thread_name_prefix="rerank-panel") as ex:
            ...
            for fut in as_completed(futs):
                scores, bbytes, ctime = fut.result()
    ...
    print(f"[iouring_panel] total_s={total_s:.3f}, io_s={io_s:.3f}, compute_s={compute_s:.3f}, vstack_s={vstack_s:.3f}, span_s={t_span_time:.3f}")
    return out, total_s, compute_s, io_s, meta, vstack_s
