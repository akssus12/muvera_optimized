...
for RC in 1000; do # 5 10 20 50 100
  ...
  python3 ours_colgemm_fullvector_streamed_iouring_panelgemm.py --p 4 --r 2 --nlist 1000 \
  -i "${FDE_PKL}" -o "${FAISS_OUT}" -rc "${RC}" -tk "${RC}" -rf 1 ${FORCE} --iouring_submit_batch 128 --io_uring_qd 128 --use_pack_half --half_policy front --build_pack_if_missing --use_tpack --build_tpack_if_missing
done